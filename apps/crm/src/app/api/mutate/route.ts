 ...
 *
 * Security:
 *   - Only mutations declared in `crmSchema.mutations` can be called.
...
export async function POST(req: NextRequest) {
  ...
  const def = (crmSchema.mutations as Record<string, import("@fluid/core").MutationDef> | undefined)?.[mutation];
  if (!def) {
    return NextResponse.json(
