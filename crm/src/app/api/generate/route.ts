...
export async function POST(req: NextRequest) {
  ...
  const key = intentKey(crmSchema.name, intent, userId);
  try {
    const refined = learn
      ? await engine.refine({
          schema: crmSchema,
    ...
    if (userId && result.cached) {
      void engine
        .checkRefresh({ userId, schemaName: crmSchema.name, lastGeneratedAt: Date.now() - latencyMs })
        .then((decision) => {
          if (decision.refresh) {
            try {
              const db = getDb();
              void enqueueRefreshJob(db, userId, crmSchema.name, decision.reason ?? "policy");
            } catch {
